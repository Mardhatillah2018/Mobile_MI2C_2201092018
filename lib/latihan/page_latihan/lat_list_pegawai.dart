...
import 'package:mi2c_mobile/latihan/page_latihan/lat_edit_pegawai.dart';
import '../model_latihan/model_pegawai_latihan.dart';

class LatihanListPegawai extends StatefulWidget {
  @override
  State<LatihanListPegawai> createState() => _LatihanListPegawaiState();
}

class _LatihanListPegawaiState extends State<LatihanListPegawai> {
  ...
  Future<void> fetchData() async {
    try {
      final response = await http.get(
          Uri.parse("http://192.168.208.154/edukasi_server/getPegawai.php"));
                                    ...
                                    actions: <Widget>[
                                      ...
                                      TextButton(
                                        onPressed: () {
                                          ...
                                          http.post(
                                            Uri.parse('http://192.168.208.154/edukasi_server/deletePegawai.php'),
                            ...
                            IconButton(
                              onPressed: () {
                                ...
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) => LatihanEditPegawai( data: entry.value),
                                  ),
                                ).then((updatedData) {
                                  if (updatedData != null) {
                ...
                ElevatedButton(
                  onPressed: () {
                    if (keyForm.currentState?.validate() == true) {
                      ...
                      http.post(
                        Uri.parse('http://192.168.208.154/edukasi_server/simpanPegawai.php'),
                        body: dataKaryawan,
                      ).then((response) {
                        // Memeriksa respons dari server
                        if (response.statusCode == 200) {
